...
pipeline {
    ...
    environment {
        ...
        BASE_IMAGE="""${sh(
                returnStdout: true,
                script: 'if [ "${run_with_race_detection}" = "" ]; then echo -n "scratch"; else echo -n "quay.io/cilium/cilium-runtime:e07db48788d78b260b92869547f9880d47c029de@sha256:5fd1ac8864fe0264c8a99150f4ddfbaf61655f0c0d8bf7748555b59d971e7108"; fi'
            )}"""
    }
